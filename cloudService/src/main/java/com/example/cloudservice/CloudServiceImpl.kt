...
class CloudServiceImpl(provideDatabase: ProvideDatabase) : CloudService {
    ...
    override fun <T : DatabaseListItem> addItemToList(item: T, vararg children: String) {
        val ref = makeReference(*children)
        ref.updateChildren(mapOf(item.id to item))
    }
    
    override fun <T : DatabaseListItem> subscribe(clazz: Class<T>, vararg children: String) =
        callbackFlow {
    ...
    override fun <T : Any> subscribeToListOf(clazz: Class<T>, vararg children: String) =
        callbackFlow<List<T>> {
            val listener = object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
